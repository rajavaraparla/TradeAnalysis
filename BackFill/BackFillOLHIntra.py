...
def backfill_intra_olh_trades(from_date,to_date = date.today()):
    ...
    for p_delta in range(delta.days):
        ...
        print("Filling TradeData for Date - " + curr_date.strftime(constants.DATE_FORMAT))
        olh_class_list = db_utils.get_olh_class_list(p_date=curr_date, dbhost=config.DB_HOST
        ...
        for index,olh_class in enumerate(olh_class_list):
            db_utils.execute_delete_query(dbhost=config.DB_HOST
                                          ...
                                          , params={constants.INTRA_OLH_TRADE_TRADE_DATE:
                                                        curr_date.strftime(constants.DATE_FORMAT)}
                                          )
            # Load table 'intra_olh_trade' with olh_trade_data
            trade_utils.insert_olh_intra_trade_db(olh_trade_class_list=olh_class_list
...
if __name__ == '__main__':
    backfill_intra_olh_trades(date.today()-timedelta(1), to_date=date.today())
    pass